 ...
 * @author Derek Mauldin (added filter email)
 */
class Filter {
	 ...
	 *
	 * @param string $string string to use - contains email address
	 * @param string $name name of attribute to filter
	 * @param int $size maximum length of the string
	 * @return mixed new string to use
	 */
	public static function filterEmail($string, $name, $size = 0) {
		// Verify the new string
		$string = trim($string);
		$string = filter_var($string, FILTER_SANITIZE_EMAIL);
		if(empty($string) === true) {
			throw(new InvalidArgumentException("$name is not a vaild email address"));
		}
